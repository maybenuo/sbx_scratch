...
table.on('toolbar(TableOne)', function (obj) {
    if (obj.event === 'search_state0' || obj.event === 'search_state1' || obj.event === 'search_state2' || obj.event === 'search_recommented1') {
...
table.on('tool(TableOne)', function (obj) {
    if (obj.event == 'setRecommented0' || obj.event == 'setRecommented1') {
        AjaxFn('/admin/works/scratch/setRecommented', {'id': obj.data.id,'r':(obj.event=='setRecommented0'?0:1) }, function (res){
            layer.msg(res['msg']);
            if ('success' == res['status']){
                _table_render.reload();
            }
        })
    } else if (obj.event === 'setState0' || obj.event === 'setState1' || obj.event === 'setState2') {
